...
class CriticSF(nn.Module):
    ...
    def forward(self, h, task, conditioner=None):
        ...
        if self.obs_type == "pixels":
            ...
            h = F.normalize(conditioner(h, actions), p=2, dim=-1) # [B*A, hidden_dim + A]
        else:
...
class SFCanonicalAgentRewardLearning(SFSimpleAgent):
    def __init__(
        self,
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_sf1 = torch.gather(target_sf1, dim=1, index=actions_expanded).squeeze()  # shape: [B, F]
            target_sf2 = torch.gather(target_sf2, dim=1, index=actions_expanded).squeeze()  # shape: [B, F]
            ...
            # conditioned_basis_features = F.normalize(self.action_conditioner(obs, action), p=2, dim=-1)
        ...
        SF1 = torch.gather(SF1, dim=1, index=actions_expanded).squeeze()  # shape: [B, F]
        SF2 = torch.gather(SF2, dim=1, index=actions_expanded).squeeze()  # shape: [B, F]
        ...
        if step % self.update_task_every_step == 0:
            metrics.update(
                self.regress_meta_grad_descent(
                    next_obs, task, reward, step, basis_features
                )
            )
