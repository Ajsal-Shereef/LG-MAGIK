...
class Vgg16(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(*list(vgg_pretrained.features.children())[:23])  # Extracts features up to conv4_3

    def forward(self, x):
        return self.features(x)  # Returns feature maps, not a flattened vector
...
def load_images_as_numpy_array(directory_path, extension_pattern='*.png'):
    """
    Loads all images of a specified pattern from a directory into a single 
    NumPy array.

    Args:
        directory_path (str): The path to the directory containing the images.
        extension_pattern (str): The file extension pattern to match (e.g., '*.jpg').

    Returns:
        np.ndarray: A NumPy array of shape (N, H, W, C), where N is the 
                    number of images, H is height, W is width, and C is 
                    channels (e.g., 3 for RGB).
        
    Raises:
        ValueError: If no images are found or if the images have different sizes.
    """
    # 1. Use glob to find all image file paths matching the pattern
    search_path = os.path.join(directory_path, extension_pattern)
    image_files = glob.glob(search_path)

    if not image_files:
        raise ValueError(f"No images found in {directory_path} matching {extension_pattern}")

    # 2. Load and convert images to NumPy arrays
    image_arrays = []
    
    # Check the size of the first image to ensure consistency
    first_img = Image.open(image_files[0])
    target_shape = np.array(first_img).shape
    first_img.close() # Close the first image file handle
    ...
    for file_path in image_files:
        try:
            ...
            if img_array.shape != target_shape:
                print(f"Skipping {file_path}: Inconsistent shape {img_array.shape}, expected {target_shape}")
                continue

            image_arrays.append(img_array)
            img.close() # Close the file handle
        except Exception as e:
            print(f"Could not load or process image {file_path}: {e}")

    # 3. Stack all image arrays into a single NumPy array
    if not image_arrays:
        raise ValueError("No valid images were loaded. Check file types and directory path.")
